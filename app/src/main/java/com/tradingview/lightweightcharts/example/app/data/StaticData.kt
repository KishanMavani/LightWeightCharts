...
import com.tradingview.lightweightcharts.example.app.view.charts.FloatingTooltipFragment.Companion.cointListLine
...
fun listFloatingTooltipSeriesLineData(): MutableList<SeriesData> {
    val lineChartCoinList: ArrayList<SeriesData> = ArrayList()
    if (!lineChartCoinList.isNullOrEmpty()) lineChartCoinList.clear()
    for (i in 0 until cointListLine.size) {
        lineChartCoinList.add(
            LineData(
                Time.Utc(cointListLine[i].time),
                cointListLine[i].close.toFloat()
            )
        )
    }
    return lineChartCoinList
...
fun listRealTimeEmulationSeriesCandlestickData(): MutableList<SeriesData> {
    ...
    for (i in 0 until cointListt.size) {
        coinListts.add(
            CandlestickData(
                Time.Utc(cointListt[i].time),
                cointListt[i].open.toFloat(),
                cointListt[i].high.toFloat(),
                cointListt[i].low.toFloat(),
